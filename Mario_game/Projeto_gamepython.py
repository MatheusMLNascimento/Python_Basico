...
def draw_window(red, yellow, red_bullets, yellow_bullets, red_hp, yellow_hp):
    ...
    yellow_hp_text = HP_FONT.render("HP: " + str(yellow_hp), 1, WHITE)
...
def yellow_handle_movement(keys_pressed, yellow):
    if keys_pressed[pygame.K_a] and yellow.x - VEL > 0:  # Direita
        yellow.x -= VEL
    if keys_pressed[pygame.K_d] and yellow.x + yellow.width < BORDER.x:  # Esquerda
        yellow.x += VEL
    if keys_pressed[pygame.K_s] and yellow.y + yellow.height < HEIGHT - 15:  # Baixo
        yellow.y += VEL
...
def main():
    red = pygame.Rect(1120, 350, SPACESHIP_WIDTH, SPACESHIP_HEIGHT)
    yellow = pygame.Rect(40, 350, SPACESHIP_WIDTH, SPACESHIP_HEIGHT)
